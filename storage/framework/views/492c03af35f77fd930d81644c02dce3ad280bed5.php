<div class="fixed <?php echo e($positionCss); ?> <?php if($hideOnClick): ?> cursor-pointer <?php endif; ?>"
    x-data="{show: false, timeout: null, duration: null}"
    <?php if($message): ?>
        x-init="() => { duration = window.livewire.find('<?php echo e($_instance->id); ?>').duration; clearTimeout(timeout); show = true;
    ...
    @new-toast.window="duration = window.livewire.find('<?php echo e($_instance->id); ?>').duration; clearTimeout(timeout); show = true;
                if( duration > 0 ) { timeout = setTimeout(() => { show = false }, duration); }"
    @click="if(window.livewire.find('<?php echo e($_instance->id); ?>').hideOnClick) { show = false; }"
    x-show="show"
...
>
    <?php if($message): ?>
        <div class="flex bg-<?php echo e($bgColorCss); ?>-500 rounded-xl border-l-4 border-<?php echo e($bgColorCss); ?>-700 py-4 px-6 shadow-md mb-2 ">
            <!-- icons -->
            <?php if($showIcon): ?>
...
<?php /**PATH C:\Users\Ali\Documents\GitHub\snappfood\snapp_food\resources\views/vendor/livewire-toast/livewire/livewire-toast.blade.php ENDPATH**/ ?>