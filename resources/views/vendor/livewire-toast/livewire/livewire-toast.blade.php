<div class="fixed {{ $positionCss }} @if ($hideOnClick) cursor-pointer @endif" x-data="{ show: false, timeout: null, duration: null }"
    ...
    @click="if(@this.hideOnClick) { show = false; }" x-show="show" @if ($transition)
    ...
    @if ($message)
        <div
            class="flex bg-{{ $bgColorCss }}-500 rounded-xl border-l-4 border-{{ $bgColorCss }}-700 py-4 px-6 shadow-md mb-2 ">
            <!-- icons -->
            @if ($showIcon)
                <div class="text-{{ $bgColorCss }}-500 rounded-full bg-{{ $textColorCss }} ml-3">
