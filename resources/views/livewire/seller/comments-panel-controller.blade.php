                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($Category as $Categories)
                                            ...
                                            <td class="border-b dark:border-dark-5 text-lg font-medium">
                                                {{ ++$Row }}</td>
                                            <td class="border-b dark:border-dark-5 text-lg font-medium">
                                                {{ $Categories->user->name }}</td>
                                            <td class="border-b dark:border-dark-5 text-lg font-medium">
                                                {{ $Categories->user->email }}</td>
                                            <td class="border-b dark:border-dark-5 text-lg font-medium">
                                                {{ $Categories->orders_id }}</td>
                                            <td class="border-b dark:border-dark-5 text-lg font-medium">
                                            ...
                                            <td class="border-b dark:border-dark-5 text-lg font-medium">
                                                {{ \Illuminate\Support\Str::limit($Categories->opinion, 20) }}</td>
                                            @php
                                                //این بخش، مقدار $Categories->opinion را با استفاده از کلاس Illuminate\Support\Str محدود می‌کند. تابع limit با دو پارامتر، یعنی متن و حداکثر طول مجاز، متن را کوتاه‌تر می‌کند. در اینجا، حداکثر طول 20 کاراکتر تعیین شده است.
                                            @endphp
                                            <td class="border-b dark:border-dark-5 text-lg font-medium">
                                                {{ $Categories->score }}</td>
                                            <td class="border-b dark:border-dark-5 text-lg font-medium">
                                                @if ($Categories->status == 'suspended')
                                                    ...
                                                    @if (\Illuminate\Support\Facades\DB::table('parent_child_comment')->where('parent_comment_id', $Categories->id)->first() != null)
                                                        @php
                                                            $parent = \Illuminate\Support\Facades\DB::table('parent_child_comment')
                                                                ->where('parent_comment_id', $Categories->id)
                                                                ->first()->child_comment_id;
                                                        @endphp
                                                ...
                                                @elseif($Categories->status == 'confirm')
                                                    <span class="text-emerald-500"> کامنت تایید و قابل رویت گردید
                                                        </span>
                                                    <button class="btn btn-outline-primary mr-3"
                                                        title="{{ \App\Models\Comment::find(DB::table('parent_child_comment')->where('parent_comment_id', $Categories->id)->get()->first()->child_comment_id)->opinion }}"
                                                        wire:click="$emit('ShowAndEdit',{{ $Categories->id }})">نمایش
                                                        پاسخ یا ویرایش
                                                    </button>
                                                @elseif($Categories->status == 'reject')
                                                    <span class="text-amber-600"> کامنت به مدیریت ارجاع داده شد </span>
                                                @endif
                                            </td>
                                            </tr>
                                        @endforeach
