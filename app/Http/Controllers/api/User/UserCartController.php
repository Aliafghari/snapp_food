...
class UserCartController extends Controller
{
    ...
    public function store(CartRequest $request)
    {
        ...
        try {
            if (empty(CartFood::where([['food_id', $request->food_id], ['cart_id', auth('api')->user()->cart]])->get()))
                return response()->json(['msg' => 'Food Exist']);
            DB::beginTransaction();

            $cart = Cart::where([['user_id', auth('api')->id()], ['state', 'FirstCart'], [
                'restaurant_detail_id',
                $food->restaurant_detail_id
            ]])->get()->first();
            if ($food->off == 1) {
            ...
            if ($cart == null) {
                $price = $foodPrice != null ? (($food->price * $request->count) + (int)$food->RestaurantDetail->ShippingCost) : (($foodPrice * $request->count) + (int)$food->RestaurantDetail->ShippingCost);
                $cart = Cart::create([
    ...
    public function update(Request $request, int $id)
    {
        ...
        $food = CartFood::where([['food_id', (int)$request->food_id], ['cart_id', $cart->id]])->first();
        //dd(CartFood::where([['food_id', (int)$request->food_id], ['cart_id', $cart->id]])->get());
        if ($food) {
            //dd($request->count);
